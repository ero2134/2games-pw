...
class AuthController extends Controller
{
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        if( !\Auth::validate($credentials) ) 
        {
            ...
            return response()->json(["abort" => trans("auth.failed")]);
        }
        $user = \Auth::getProvider()->retrieveByCredentials($credentials);
        if( $user->isUnconfirmed() ) 
        {
            return response()->json(["abort" => trans("app.please_confirm_your_email_first")]);
        }
        if( $user->isBanned() ) 
        {
            return response()->json(["abort" => trans("app.your_account_is_banned")]);
        }
    ...
    public function apiLogin($game, $token, $mode)
    {
        ...
        $us = User::where("api_token", "=", $token)->get();
        if( isset($us[0]->id) ) 
        {
    ...
    {
        return (session("auth.2fa.id") ? view("auth.token") : redirect("/"));
    }
    public function postToken(Request $request)
    {
        ...
        {
            return redirect()->to("/")->withErrors(trans("app.2fa_token_invalid"));
        }
        \Auth::login($user);
        event(new LoggedIn());
    ...
    public function postRegister(RegisterRequest $request)
    {
        ...
        $data["username"] = $this->generateUsername($request->email);
        $user = $this->users->create(array_merge($data, ["status" => $status]));
        ...
        {
            return response()->json(["success" => "success", "link" => route("frontend.game.list", ["verify_email" => true])], 200);
        }
        else
        {
            \Auth::login($user, true);
            return response()->json(["success" => "success", "link" => route("frontend.game.list")], 200);
        }
    }
    ...
    public function checkUsername($username)
    {
        ...
        while( !$generated ) 
        {
            $count = User::where("username", $generate)->count();
