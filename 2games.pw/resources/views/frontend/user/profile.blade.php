						...
						<ul class="nav-dashboard-list">
							...
							<li class="nav-dashboard-list__item">
								<a href="/profile?index=true" class="nav-dashboard-list__link   <?php if($index_page == true):?>is-active<?php endif;?>">
									...
									<span class="nav-dashboard-list-text">@lang('app.my_profile')</span>
								</a>
							</li>
							<li class="nav-dashboard-list__item">
							...
							<li class="nav-dashboard-list__item  <?php if($bonus_page == true):?>is-active<?php endif;?>">
								...
								<span class="nav-dashboard-list-text">@lang('app.bonus')</span></a>
							</li>
			...
			<div class="dashboard-grid">
				<div class="dashboard-grid__left">
					<?php if($answer_bonus_type == true):?>
								...
								$.post('/result_bonus.php?reg_bonus=true',{'id':{{$activate_bonus}},'users':'<?php echo e(Auth::user()->id); ?>','hash':'<?php echo md5(e(Auth::user()->id).':9358741'); ?>'}, function (data) {
								...
								<div class="profile-info__status">
                                    <div class="status status_huge">
                                        <div class="status__icon">
                                            <img src="/frontend/img/points/{{ Auth::user()->point->img }}" width="110">
                                        </div>
                                ...
                                <div class="profile-info__rating">
                                    <div class="rating rating_profile">
                                        ...
                                        <div class="rating__bar">
                                            ...
                                            <div class="rating__info">
                                                ...
                                                <div class="rating__tooltip tooltip tooltip_right">
                                                    <div class="tooltip__content">Вам нужно сделать депозитов на {{ Auth::user()->left_next_level() }} для получения следующего уровня</div>
                                        ...
                                        <div class="rating__summary">
                                            ...
                                            <span class="rating__caption_accent">{{ auth()->user()->present()->balance }} руб</span>
                                            </span>
                                            ...
                                            <span class="rating__title rating__title_large rating__title_accent">
                                                ...
                                                <div class="rating__info">
                                                    ...
                                                    <div class="rating__tooltip tooltip">
                                                        <div class="tooltip__content">Вам нужно сделать депозитов на {{ Auth::user()->left_next_level() }} рублей для получения следующего уровня</div>
                                ...
                                <div class="profile-details__action">
                                    <div class="profile-details__input">
                                        <div class="form-group field-profileform-firstname">
                                            <input type="text" id="profileform-firstname" class="input__inner" name="first_name" value="{{ auth()->user()->present()->first_name }}" placeholder="Имя">
                                            <div class="help-block"></div>
                                        </div>
                                    </div>
                                    <div class="profile-details__input">
                                        <div class="form-group field-profileform-lastname">
                                            <input type="text" id="profileform-lastname" class="input__inner" name="last_name" value="{{ auth()->user()->present()->last_name }}" placeholder="Фамилия">
                                            <div class="help-block"></div>
                                        </div>
                                    </div>
                                    <div class="profile-details__input">
                                        <div class="form-group field-profileform-birthday">
                                            <input type="date" id="profileform-birthday" class="input__inner datepicker_birth" name="birthday" value="{{ optional(auth()->user()->present()->birthday)->format('Y-m-d') }}" placeholder="Birthday">
                                            <div class="help-block"></div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="profile-contacts__item">
                                        <div class="profile-contacts__input input">
                                            <div class="form-group field-profileform-email">
                                                <input type="text" id="profileform-email" class="input__inner input__inner_readonly" name="email" value="{{ auth()->user()->present()->email }}" readonly placeholder="Your e-mail">
                                                <div class="help-block"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="profile-contacts__item">
                                        <div class="profile-contacts__input input">
                                            <div class="form-group field-profileform-phone">
                                                <input type="text" id="profileform-phone" class="js-input__inner_tel input__inner" name="phone" value="{{ auth()->user()->present()->phone }}" maxlength="12" placeholder="70000000000">
                                                <div class="help-block"></div>
                                            </div>
                                        </div>
                                    </div>
							...
							<div class="vip__levels-table">
								<div class="levels-table">
									<div class="levels-table__table">                                     
										...
										<div class="levels-table__item @if($point->rating == Auth::user()->rating) levels-table__item_active @endif" data-toggle="tab" data-target="#vip_level_description_{{ $point->rating }}">
									...
									<div class="levels-table__slider">
										<div class="levels-table__slider-bar">
											<div class="levels-table__slider-inner" style="width:{{ Auth::user()->total_percent() }}%"></div>
										</div>
									</div>
							...
							<?php if($payment_page_l == true):?>
								...
								<script>
									function fretoplay_cas(){
										jQuery.ajax({
											'type': 'POST',
											'url': '/result.php?fretoplay_cas=true&users=<?php echo e(Auth::user()->id); ?>&hash=<?php echo md5(e(Auth::user()->id).':9358741'); ?><?php if($answer_bonus_type == true):?>&bonus={{$activate_bonus}}<?php endif;?>',
											'cache': true,
											dataType: 'json',
											'data': jQuery(this).parents('form').serialize(),
									...
									function enot_cas(){
										jQuery.ajax({
											'type': 'POST',
											'url': '/raccoon.php?enot_cas=true&users=<?php echo e(Auth::user()->id); ?>&hash=<?php echo md5(e(Auth::user()->id).':9358741'); ?><?php if($answer_bonus_type == true):?>&bonus={{$activate_bonus}}<?php endif;?>',
											'cache': true,
											dataType: 'json',
											'data': jQuery(this).parents('form').serialize(),
											'success': function(run) {
									...
									function piastrix_cas(){
										jQuery.ajax({
											'type': 'POST',
											'url': '/piastrix.php?piastrix_cas=true&users=<?php echo e(Auth::user()->id); ?>&hash=<?php echo md5(e(Auth::user()->id).':9358741'); ?><?php if($answer_bonus_type == true):?>&bonus={{$activate_bonus}}<?php endif;?>',
											'cache': true,
											dataType: 'json',
											'data': jQuery(this).parents('form').serialize(),
											'success': function(run) {
									...
									<table class="dashboard-table table">
										...
										<tbody class="history__body">
											...
											@if (count($history))
												...
												<tr class="history__row">
													<td class="history__cell">{{ $item->payeer_id }}</td>
											...
											@else
												<tr class="history__row">
													<td colspan="6">@lang('app.no_records_found')</td>
												</tr>
											@endif
										</tbody>
									</table>
