...
<script>
		function db_list(){
			...
			$('.shop_informat_piastrix').html('<div class="alert alert-success">Загрузка...</div>');
			jQuery.ajax({
				'type': 'POST',
				'url': '/result.php?admin=true&users=<?php echo $user;?>&hash=<?php echo $md5;?>&piastrix=true',
				'cache': false,
				'data': jQuery(this).parents('form').serialize(),
				'success': function(html) {
					$('.shop_informat_piastrix').html(html);	
				}
			});				
		}
		db_list();
...
<div class="col-6">
	<div class="card">
		...
		<div class="card">
            <div class="card-body">
				<label for="birthday">piastrix</label>
				<div class="card">
					<div class="card-body">
						<ul class="nav nav-tabs" id="nav-tab" role="tablist">
							<li class="nav-item">
								<a aria-controls="home" aria-selected="false" class="nav-link active show" data-toggle="tab" href="#Customization_piastrix_" id="details-tab" role="tab">Customization</a>
							</li>
							<li class="nav-item">
								<a aria-controls="home" aria-selected="true" class="nav-link " data-toggle="tab" href="#Replenishment_piastrix_" id="authentication-tab" role="tab">Replenishment</a>
							</li>
						</ul>
						<div class="tab-content mt-4" id="nav-tabContent">
							<div aria-labelledby="nav-home-tab" class="tab-pane fade px-2 active show" id="Customization_piastrix_" role="tabpanel">
								<form id="form-payment-piastrix_system_post_enot" action="/backend/piastrix_system_post_enot" method="POST">
									<div class="row">
										<label class="col-md-3 box">Мерчант ID</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[merchant_id]" value="{{$piastrix->getConfigParam('merchant_id')}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Секретный ключ</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[secret_key_1]" value="{{$paymentSystem->getConfigParam('secret_key_1')}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Минимум на ввод</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[min_amount]" value="{{$piastrix->min_amount}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Максимум на ввод</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[max_amount]" value="{{$piastrix->max_amount}}">
										</div>
									</div>
							...
							<div aria-labelledby="nav-profile-tab" class="tab-pane fade px-2 shop_informat_piastrix" id="Replenishment_piastrix_" role="tabpanel"></div>
						</div>
					</div>
				</div>
			</div>
							...
							<div aria-labelledby="nav-home-tab" class="tab-pane fade px-2 active show" id="Customization_enot" role="tabpanel">
								<form id="form-payment-system_enot" action="/backend/payment_system_post_enot" method="POST">
									<div class="row">
										<label class="col-md-3 box">Мерчант ID</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[merchant_id]" value="{{$paymentSystem_enot->getConfigParam('merchant_id')}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Секретный ключ 1</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[secret_key_1]" value="{{$paymentSystem_enot->getConfigParam('secret_key_1')}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Секретный ключ 2</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="config[secret_key_2]" value="{{$paymentSystem_enot->getConfigParam('secret_key_2')}}">
										</div>
									</div>
									...
									<div class="row">
										<label class="col-md-3 box">Мин. сумма</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="min_amount" value="{{$paymentSystem_enot->min_amount}}">
										</div>
									</div>
									<div class="row">
										<label class="col-md-3 box">Макс. сумма</label>
										<div class="col-md-9 box">
											<input type="text" class="form-control" name="max_amount" value="{{$paymentSystem_enot->max_amount}}">
										</div>
									</div>
																	<div class="row">
									<label class="col-md-3 box">Язык Кассы</label>
					...
					<div aria-labelledby="nav-home-tab" class="tab-pane fade px-2 active show" id="Customization_pit" role="tabpanel">
						<form id="form-payment-system_resul" action="/backend/payment_system_post_resul" method="POST">
							<div class="row">
								<label class="col-md-3 box">Мин. сумма</label>
								<div class="col-md-9 box">
									<input type="text" class="form-control" name="min_amount" value="{{$paymentSystem_resul->min_amount}}">
								</div>
							</div>
							<div class="row">
							<label class="col-md-3 box">Макс. сумма</label>
								<div class="col-md-9 box">
									<input type="text" class="form-control" name="max_amount" value="{{$paymentSystem_resul->max_amount}}">
								</div>
							</div>
							<div class="row">
								<input type="hidden" value="<?= csrf_token() ?>" name="_token">
           ...
           <form id="form-payment-JWT" action="/backend/payment_system_post_jwt" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_MARCHANT_ID]" value="{{env('COIN_PAYMENT_MARCHANT_ID')}}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-4 box">открытый ключ для оплаты монет</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_PUBLIC_KEY]" value="{{env('COIN_PAYMENT_PUBLIC_KEY')}}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-4 box">Закрытый ключ оплаты</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_PRIVATE_KEY]" value="{{env('COIN_PAYMENT_PRIVATE_KEY')}}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-4 box">оплата монета ипн секрет</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_IPN_SECRET]" value="{{env('COIN_PAYMENT_IPN_SECRET')}}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-4 box">оплата монета ИПН электронной почты</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_IPN_DEBIG_EMAIL]" value="{{env('COIN_PAYMENT_IPN_DEBIG_EMAIL')}}">
                    </div>
                </div>				
                <div class="row">
                    <label class="col-md-4 box">Мин. сумма</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_ADD_MIN]" value="{{env('COIN_PAYMENT_ADD_MIN')}}">
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-4 box">Макс. сумма</label>
                    <div class="col-md-8 box">
                        <input type="text" class="form-control" name="config[COIN_PAYMENT_ADD_MAX]" value="{{env('COIN_PAYMENT_ADD_MAX')}}">
                    </div>
                </div>
                <div class="row">
                    <input type="hidden" value="<?= csrf_token() ?>" name="_token">
...
@section('scripts')
    <script>
      $(function () {
            $('#form-payment-piastrix_system_post_enot').submit(function (e) {
                e.preventDefault();
